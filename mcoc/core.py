...
async def cog_load(self):
    ...
    # Debug: list what the tree currently exposes (top-level and counts)
    try:
        top_names = [c.name for c in self.bot.tree.get_commands()]
        log.debug("Local tree top-level commands after add: %s", top_names)
        for cmd in self.bot.tree.get_commands():
            children = getattr(cmd, "children", None) or getattr(cmd, "commands", None)
            log.debug("About to sync command %s children=%s", cmd.name, [ch.name for ch in children] if children else [])
    except Exception:
        log.exception("Failed to enumerate bot.tree.get_commands() during cog_load debug")

    # Force a sync and capture errors (attempt global sync first; fallback to test guild if needed)
    try:
        # Try a global sync (may be rate-limited); if it returns empty, attempt a guild sync for immediate verification
        res = await self.bot.tree.sync()
        log.debug("TRACE: tree sync completed; result: %s", res)
        count = len(res) if hasattr(res, "__len__") else 0
        log.info("Application command tree sync completed; %d entries", count)

        # If global sync produced no entries, attempt a targeted guild sync to the CDT test guild for immediate verification
        if count == 0:
            try:
                from discord import Object as DiscordObject
                GID = 215271081517383682  # CDT test guild
                log.debug("Global sync returned 0 entries; attempting guild sync to %s for verification", GID)
                gres = await self.bot.tree.sync(guild=DiscordObject(id=GID))
                log.info("Post-load guild sync result: %d entries", len(gres) if hasattr(gres, "__len__") else 0)
            except Exception:
                log.exception("Post-load guild sync attempt failed")
    except Exception:
    ...
    async def cog_unload(self):
        ...
        try:
            # Attempt to remove both active and disabled entries for our groups
            tree = self.bot.tree
        ...
        # Final debug: log remaining RedTree internal maps for post-unload inspection
        try:
            tree = self.bot.tree
            log.debug("Post-unload tree.get_commands(): %s", [c.name for c in tree.get_commands()])
            log.debug("Post-unload _global_commands keys: %s", list(getattr(tree, "_global_commands", {}) or {}))
            log.debug("Post-unload _disabled_global_commands keys: %s", list(getattr(tree, "_disabled_global_commands", {}) or {}))
        except Exception:
